...
const WalletForm = <T extends FieldValues>({
  schema,
  defaultValues,
  formType,
  onSubmit,
}: FormProps<T>) => {
  const { onClose, id } = useOpenWallet();
  ...
  const buttonText = formType === "CREATE" ? "Create wallet" : "Save changes";
